...
class EnergyServiceTest {
    ...
    private static final BigDecimal energy = new BigDecimal("200");
    private static final String id = "2";


    @BeforeEach
    public void setUp() {
        cut = new EnergyService();

        //mock
        MockitoAnnotations.initMocks(this);

        building = Mockito.mock(Building.class);
        Mockito.when(building.calculateEnergy()).thenReturn(energy);
    ...
    void getPowerShouldReturnObjectWithProperNameAndType() {
        //when
        EnergyInformation actual = cut.calculateEnergy(building, id, LocalizationType.BUILDING);

        //then
        Mockito.verify(building).calculateEnergy();
        ...
        Assertions.assertEquals(actual.getEnergy(), energy);
    }
}